/*
    ...
    VARIABLE NAME: AiCasHelicopterStrike
    TYPE: None
    ACTIVATION: Anybody
...
[] spawn {
    private _blufor_helicopter = "B_Heli_Attack_01_F";
    private _opfor_helicopter = "O_Heli_Attack_02_F";
    private _indep_helicopter = "I_Heli_light_03_dynamicLoadout_F";
    ...
    private _fnTargetIdentification = {
        ...
        if (_show_debug_messages) then {
            ...
            systemchat format ["DEBUG - Group leader '%1' (%2) requested CAS HELICOPTER STRIKE at enemy '%3' (%4) position (Grid: %5).", name _observer, side _observer, _targetType, side _target, _targetGrid];
        };

        [_observer, _target] call _fnCASHelicopterStrike;
    };

    private _fnCASHelicopterStrike = {
        params ["_observer", "_target"];
		...
		if (side _observer == west) then {_vehicleClass = _blufor_helicopter};
        if (side _observer == east) then {_vehicleClass = _opfor_helicopter};
        if (side _observer == resistance) then {_vehicleClass = _indep_helicopter};
        ...
        _helicopter = createVehicle [_vehicleClass, _spawnPos, [], 0, "FLY"];
        _helicopter setPosATL [(getPosATL _helicopter select 0), (getPosATL _helicopter select 1), (getPosATL _helicopter select 2) + _spawnHeight];
        _helicopter setDir (_helicopter getDir _target);
        createVehicleCrew _helicopter;

        waitUntil {alive driver _helicopter};

        _showDebug = true;

        while {alive driver _helicopter} do {
            _helicopter setFuel 1;
            _helicopter setSkill 1;
            ...
            _helicopter flyInHeight _spawnHeight;

            if (_show_debug_messages) then {
                hint format ["Helicopter altitude: %1m\nTarget distance: %2m", (getPosATL _helicopter select 2) toFixed 2, (_helicopter distance _target) toFixed 2];
            };
            ...
            if ((alive _target) && ((_target isKindOf "Man" && isNull objectParent _target) || (_target isKindOf "AllVehicles" && {alive _x} count (crew _target) > 0))) then {
                _helicopter doMove (getPosATL _target);
                
                if (_helicopter distance _target <= _engageDistance) then {
                    _helicopter reveal _target;
                    _helicopter doTarget _target;
                    _helicopter doFire _target;
                } else {
                    _helicopter forgetTarget _target;
                };
            } else {
                _helicopter doMove (_spawnPos);
                
                if (_helicopter distance _spawnPos <= 1000) exitWith {
                    {deleteVehicle _x} forEach crew _helicopter;
                    deleteVehicle _helicopter;
                };

                if ((_show_debug_messages) && (_showDebug)) then {
                    systemchat format ["DEBUG - The CAS HELICOPTER (%1) neutralized the target (%2)!", _observerSide, _targetSide];
                    _showDebug = false;
                };
            };
        ...
        if (_show_debug_messages) then {
            ...
            if ((alive _target) && ((_target isKindOf "Man" && isNull objectParent _target) || (_target isKindOf "AllVehicles" && {alive _x} count (crew _target) > 0))) then {
                ...
                if !(alive _helicopter) then {
                    systemchat format ["DEBUG - The CAS HELICOPTER (%1) was shot down.", _observerSide];
                };
            };
        };
    };
